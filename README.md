...
- [第十五章：CLAUDE.md 记忆花园](#第十五章claudemd-记忆花园)
- [第十六章：隐藏命令与彩蛋](#第十六章隐藏命令与彩蛋)
- [第十七章：源码泄露的完整故事](#第十七章源码泄露的完整故事)
- [延伸阅读](#延伸阅读)
...
你有没有遇到过这样的情况：每次开始一段新的 Claude 对话，你都要先解释一遍"我是做前端的，用 React，不要给我推荐 Vue"？这就像你每次去同一家餐厅，都要重新告诉服务员"我不吃香菜"。

**CLAUDE.md 就是那个「记住你偏好」的机制。** 它是一个放在项目根目录的普通文本文件，Claude 每次启动时都会自动读取它，把里面的内容作为「前置上下文」加载进来。

### 最佳实践

1. **写「为什么不」，而不只是「是什么」** — `我用 FastAPI——不要推荐 Flask，我们从 Flask 迁移过来的` 比 `我用 FastAPI` 更有效
2. **只记录 Claude 无法自动推断的内容** — Claude 能读 `package.json` 知道你用了哪些库，CLAUDE.md 应该只记录看不见的东西：决策背景、团队约定、历史教训
3. **用指令代替散文** — 「明确优于聪明」比「我通常更喜欢写明确易懂的代码……」少用 7 倍 Token
4. **定期修剪，保持精简** — 100-200 行是最佳实践，超过 500 行需要认真考虑是否值得

### 加载优先级

| 文件位置 | 作用范围 | 典型用途 |
|---------|---------|--------|
| `~/.claude/CLAUDE.md` | 全局（所有项目）| 个人偏好、通用约束 |
| `项目根目录/CLAUDE.md` | 当前项目 | 项目技术栈、团队规范 |
| `.claude/settings.json` | 当前项目 | 权限配置、工具开关 |
| `子目录/CLAUDE.md` | 该子目录 | 模块级特殊说明 |
...
Claude Code 内置了一个「Buddy 系统」——18 种有名字、有性格的 AI 伴侣角色，对应不同的动物形象（猫头鹰 Archie、狐狸 Ember、熊 Bruno……）。这个功能在公开版本中尚未完全开放，但代码已经完整实现。

### 愚人节彩蛋

在 `companion.ts` 的第一行：`const SALT = 'friend-2026-401'`。`2026-401` 是 **2026 年 4 月 1 日**——愚人节。工程师把源码泄露的第二天藏进了代码里。

### ANT-only 隐藏命令

| 命令 | 功能 |
|-----|-----|
| `/ultraplan` | 多 Agent 并行规划，生成结构化设计文档 |
| `/ultrareview` | 多 Agent 代码审查，从多角度分析代码质量 |
| `/bughunter` | 系统性漏洞搜索，多 Agent 并行扫描 |
| `/ctx_viz` | 可视化当前上下文窗口使用情况 |

这些命令在公开版本中被 Feature Flag 禁用，但代码逻辑完整存在于源码中。

### 89 个特性标志

Claude Code 有 89 个 Feature Flag，控制着从 UI 细节到核心功能的方方面面，分为：多 Agent 功能（~15 个）、安全与权限（~20 个）、UI 与交互（~18 个）、ANT-only 命令（~12 个）、遥测与调试（~10 个）、实验性功能（~14 个）。
...
2026 年 3 月 31 日凌晨 2:14，研究者 Chaofan Shou 在 Claude Code 的 npm 包中发现了一个指向 Cloudflare R2 存储桶的 URL。这个 URL 指向的是 Source Map 文件——一种包含完整原始 TypeScript 源码的调试文件。Anthropic 在发布时意外地把这个文件的地址留在了公开的 npm 包里。

### 技术原因

Source Map 是一个映射文件，记录了「压缩后的代码」对应「原始 TypeScript 源码」的位置关系。Claude Code 的构建脚本在生成 npm 包时，把 Source Map 文件上传到了 Cloudflare R2，并在 JavaScript 文件末尾留下了访问 URL。任何人拿到这个 URL 就能下载到完整的可读源码。

### 泄露内容

- 完整 TypeScript 源码（约 3 万行）
- 系统提示词（约 5000 Token）
- 所有 17 个工具的完整实现
- 三层权限模型的完整逻辑
- 89 个特性标志及其默认值

**没有泄露**：Claude 模型权重、用户对话数据、API 密钥、服务端代码。

### 工程启示

生产环境不应包含 Source Map URL；npm 包内容应该被视为公开信息；构建流程需要安全审计。
